...
【参考知識（社内Knowledge採用済み）】
Xアルゴリズムの優先シグナル順位は「リプライ > 引用/リポスト > いいね」。
新規フォロワー獲得は自アカウント内の工夫だけでなく、同ジャンルの他アカウントの
投稿に早期リプライすることでその閲覧者に発見してもらう経路も有効とされる
（現状HAL/すなくんとも未着手）。フォロー転換が弱い場合の指示に、この観点を
選択肢として含めてよい。ただし実行可否・頻度は社長判断が必要なため、
「実行してください」ではなく「社長への提案」として明記すること。
