...
from scripts.common.sora_report import get_sora_instructions
...
- 日本語と簡体字中国語のバイリンガル投稿。繁体字は使わない。

【投稿の構成（2段構成）】
- 1段目: 日本語本文＋簡体字訳のみ（ハッシュタグは含めない）
- 2段目: ハッシュタグのみ（hashtagsに配列で出力）

【投稿ルール】
- Xの実際の文字数上限は280ユニット（漢字・ひらがな・簡体字・絵文字などの
  全角文字は1文字=2ユニットとしてカウントされる。半角の英数字やハッシュタグの
  記号部分は1文字=1ユニット）。post_text（日本語部分＋簡体字訳）とハッシュタグを
...
{"post_text":"日本語本文＋簡体字訳（ハッシュタグなし）", "hashtags":["タグ1","タグ2","タグ3"], "media_type":"image|video|none", "media_prompt":"画像/動画生成プロンプト（noneなら空文字）", "photo_context":"work|private"}"""
...
def _build_context_block() -> str:
    """直近の勝ち/負けパターン＋ソラの改善指示を動的取得してプロンプト末尾に追加する。
    取得失敗・データ不足時は空文字（既存挙動維持）。
    ソラ注入: growth_report_latest.json の【HALへの指示】を毎回反映し、
    投稿品質の自動改善ループを実現する（2026-07-17）。"""
    parts = []
    win = get_win_patterns(account="HAL")
    if win:
        parts.append("【直近の自アカウント勝ち/負けパターン】\n" + win)
    sora = get_sora_instructions("HAL")
    if sora:
        parts.append("【ソラ（グロースアナリスト）からの改善指示（昨日の分析）】\n" + sora)
    if not parts:
        return ""
    return (
        "\n\n" + "\n\n".join(parts)
        + "\n\n上記は参考情報。2段構成・簡体字併記・URL直貼り禁止の運用ルールは絶対遵守。"
    )
...
def run():
    ...
    # 1段目(日本語+簡体字、マモル審査済み) + 2段目(ハッシュタグ) を組み立てる。
    ...
    qa_summary = (
        "✅ 自動監査 通過済み: AI開示語 / MIMOMI早期言及 / URL直貼り / 繁体字 / "
