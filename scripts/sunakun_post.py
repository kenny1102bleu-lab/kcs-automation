...
- 「今だけ」「限定」等の煽り・焦らせ系の訴求表現は使わない（外部事例でフォロワー
  反応の低下が確認されている）。商品自体の名称・仕様として「限定モデル」等が
  含まれる場合はそのまま書いてよいが、購入を急かす文脈での多用は避ける
- 「フォロー訴求あり」と指示された場合のみ、フォローすると新商品情報を
  見逃さない、といったニュアンスの一言を本文に自然に混ぜてよい。指示が
  無い場合は通常通り書くこと
...
# フォロー訴求フックを混ぜる確率（社長確認済み、2026-07-12: 4-5投稿に1回程度）
FOLLOW_HOOK_PROBABILITY = 0.22
...
def _build_context_block() -> str:
    """バズ型・勝ちパターン・ソラの改善指示を動的取得してプロンプト末尾に追加するブロックを返す。
    全部Noneなら空文字（既存挙動を維持）。
    ソラ注入: growth_report_latest.json の【すなくんへの指示】を毎回反映し、
    ...
    sora = get_sora_instructions("SUNAKUN")
    if sora:
        blocks.append("【ソラ（グロースアナリスト）からの改善指示（昨日の分析）】\n" + sora)
    if not blocks:
        return ""
...
def run():
    ...
    # への対応。毎回だと宣伝っぽくなるため確率式にする（HALより頻度高め）。
    if random.random() < FOLLOW_HOOK_PROBABILITY:
        user_message += "\n※今回はフォロー訴求ありで書いてください（宣伝っぽくならないよう、本文に1文自然に混ぜる）。"
