...
## 8. 追記（2026-07-08）: このドキュメントの記述は本番に反映されていなかった

朝礼（`morningBriefing`）に「⚠️ 勤怠GAS URLが設定されていません」という警告が出続けていることを社長が指摘。`clasp pull` で本番の `GAS_KCS合同会社_Backend.js` を取得して確認したところ、本ドキュメントが「削除済み」と記録していた8箇所（§2の表）が**すべてそのまま残っていた**。パッチが `clasp push` されなかったか、後で巻き戻ったと思われる。

改めて §2 の8箇所を削除し、`clasp push -f` で本番に反映済み（2026-07-08 08:59 push確認）。

**教訓:** この種のrationaleドキュメントは「作業計画・意図」の記録であり、本番コードへの反映を保証しない。実際の挙動を調査する際は、リポのドキュメントだけでなく `clasp pull` で本番コードを直接確認すること。
